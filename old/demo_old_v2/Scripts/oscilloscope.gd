...
@export var synth_frames : PackedVector2Array = [] : set = draw_oscilloscope

func draw_oscilloscope(_v:PackedVector2Array):
	synth_frames = _v
	queue_redraw()
...
func _draw():
	...
	for i in synth_frames.size():
		...
		point.y = (synth_frames[i].y*size.y/2)+size.y/2
