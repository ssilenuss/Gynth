...
void Osc::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("get_to_fill"), &Osc::get_to_fill);
	ClassDB::bind_method(D_METHOD("set_to_fill", "_value"), &Osc::set_to_fill);
	ClassDB::add_property("Osc", PropertyInfo(Variant::INT, "to_fill"), "set_to_fill", "get_to_fill");
...
void Osc::_process(double delta){
    if (playback==nullptr){
    //UtilityFunctions::print("playback == nullptr");
    return;
    }
    else{
        fill_buffer();
...
void Osc::set_to_fill(int _value){
	to_fill = _value;
}
int Osc::get_to_fill(){
	return to_fill;
